...
for (i in 1:length(physeq_list)){
  ...
  if(type == "vsearch") {
    full_tab <- full_tab %>%
      gather(key = "Sample", value = "Reads", as.character(sample_data(physeq)$sample.id)) %>%
...
ggplot(dada2_qc %>% 
  ...
  labs(x = "Non-chimeric Denoised Reads", y = "Number of ASVs", color = params$colorby) +
  ggtitle("DADA2 QC Metrics")
```
